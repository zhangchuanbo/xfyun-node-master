...
const base = '../src/sdk/bin/msc/res/xtts';
const resource={
    en:path.resolve(__dirname,`${base}/common_en.jet`),
    zh:path.resolve(__dirname,`${base}/common.jet`),
    xiaoyan:path.resolve(__dirname,`${base}/xiaoyan.jet`),
    xiaofeng:path.resolve(__dirname,`${base}/xiaofeng.jet`),
}
...
tts.speak = (info={})=>{
    ...
    const defalut_params = {engine_type:"purextts",voice_name:"xiaoyan",text_encoding:"UTF8",sample_rate:"16000",speed:"50",volume:"50",pitch:"50",rdn:"2"};
    const synthetic_params = {...defalut_params,...params};
    let hasttspath = false;
    if(["xiaoyan","xiaofeng"].includes(synthetic_params.voice_name)) {
        default_res[0] = resource[synthetic_params.voice_name];
        hasttspath = true;
    }
    if(synthetic_params.lang) {
        hasttspath = true;
        default_res[1] = resource[synthetic_params.lang];
        delete synthetic_params.lang;
    }
