...
const base = path.resolve(__dirname.replace('app.asar', 'app.asar.unpacked'), `./src/sdk/bin/msc/res/${process.platform}`);
const resource = {
...
tts.speak = (info = {}) => {
    ...
    const defalut_params = { engine_type: process.platform === 'linux' ? "purextts" : "local", voice_name: "xiaoyan", text_encoding: "UTF8", sample_rate: "16000", speed: "50", volume: "50", pitch: "50", rdn: "2" };
    ...
    // del log and file
    rimraf.sync(path.resolve(`./msc`))
    rimraf.sync(path.resolve(`./${filename}`))
    return ttsInfo;
}
